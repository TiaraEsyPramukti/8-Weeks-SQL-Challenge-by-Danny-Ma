...
WITH runner_orders_clean AS(
	SELECT
		...
		CAST(CASE WHEN LEFT(distance, patindex('%[0-9][^.0-9]%', distance+'%[0-9]')) = 'null' THEN NULL
			ELSE LEFT(distance, patindex('%[0-9][^.0-9]%', distance+'%[0-9]')) END AS NUMERIC(7,2)) AS distance,
		CAST(CASE WHEN LEFT(duration, patindex('%[0-9][^.0-9]%', duration+'%[0-9]')) = 'null' THEN NULL
			ELSE LEFT(duration, patindex('%[0-9][^.0-9]%', duration+'%[0-9]')) END AS NUMERIC(7,2)) AS duration,
...
SELECT
	...
	COUNT(CAST(pizza_name AS NVARCHAR(100))) AS ordered
FROM customer_orders
JOIN pizza_names
ON customer_orders.pizza_id = pizza_names.pizza_id
GROUP BY CAST(pizza_name AS NVARCHAR(100)), customer_id;  
